...
class App {
  ...
  private authMiddleware = async (
    ...
    // TODO: Update Access token from cache
    console.log(
      `${request.method} ${request.path} ${String(request.query.key)}`
    );
    // const auth = new Auth();
    let enumString: any;
    //if(!this._auth.hasAccessToken())
    // console.log('Did not have token: fetching from server');
    enumString = await this._auth
      .loginApiKey(String(request.query.key))
      .catch((err) => {
        console.log(err);
      });
    ...
    if (enumString === 'AUTH_SUCCESS') {
      ...
      response
        .status(400)
        .json('Failed to authenticate request. Please ensure valid apikey');
    }
  };
  ...
  private initializeControllers = (controllers: ControllerInterface[]) => {
    ...
    this.app.use('/', (req: express.Request, res: express.Response) => {
      res.status(200).render(path.join(__dirname, '/public/views/welcome'));
    });
  };
